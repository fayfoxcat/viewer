 ...
 * 【目录区】文件树渲染模块
 */
window.ViewerFileTree = (function () {
    'use strict';
    ...
    function buildFileRow(opts) {
        ...
        return `
          <div class="file-row" title="${window.ViewerUtils.escapeHtml(opts.tooltip || "")}">
            <div class="file-col file-col-name" ${indentStyle}>
              ${expanderHtml}
              <span class="file-icon">${opts.icon || ""}</span>
              <span class="file-label" title="${window.ViewerUtils.escapeHtml(opts.name || "")}">${window.ViewerUtils.escapeHtml(opts.name)}</span>
            </div>
            <div class="file-col file-col-time" title="${window.ViewerUtils.escapeHtml(opts.mtime || "-")}">${window.ViewerUtils.escapeHtml(opts.mtimeShort || "-")}</div>
            <div class="file-col file-col-size" title="${window.ViewerUtils.escapeHtml(opts.size || "-")}">${window.ViewerUtils.escapeHtml(opts.size || "-")}</div>
          </div>
        `;
    }
    ...
    function renderDirectoryChildren($ul, children, depth, expandedPathsSet, expandedZipPathsSet) {
        ...
        sortedList.forEach(function (f) {
            ...
            const isArchive = !isDir && window.ViewerUtils.isArchiveFileName(f.name);
            ...
            if (!isDir) {
                ...
                const currentSelectedIds = window.ViewerSelectionManager ? window.ViewerSelectionManager.getSelectedIds() : new Set();
            ...
            $li.html(buildFileRow({
                name: f.name,
                size: isDir ? "-" : window.ViewerUtils.formatFileSize(f.size),
                mtime: window.ViewerUtils.formatDate(f.lastModified),
                mtimeShort: window.ViewerUtils.formatDateShort(f.lastModified),
                tooltip: f.name,
                depth,
