 ...
 * 负责日志内容区域的右键菜单功能
 */
window.LogViewerContextMenu = (function() {
    'use strict';
    ...
    function bindEvents() {
        // 阻止默认右键菜单
        $(document).on('contextmenu', '#log-content-actual', function(e) {
            e.preventDefault();
            showContextMenu(e);
            return false;
        });
        ...
        // 滚动时关闭菜单
        $('#log-content-actual').on('scroll', function() {
            hideMenu();
        });
        ...
        $(document).on('keydown', function(e) {
            ...
            if (e.ctrlKey && e.key === 'c') {
                ...
                if (selectedText && $(selection.anchorNode).closest('#log-content-actual').length > 0) {
                    // 静默复制，不显示通知
                }
            }
    ...
    function showDefaultMenu(e) {
        const activeFileName = window.LogViewerUIState ? window.LogViewerUIState.getActiveFileName() : null;
        const isMarked = currentLineNumber && markedLines.has(currentLineNumber);
        
        // 检查是否有高亮内容
        const hasHighlights = $('.log-line .log-txt mark.log-highlight').length > 0;
    ...
    function performCopy() {
        ...
        navigator.clipboard.writeText(currentSelection).then(() => {
            ...
            if (window.LogViewerNotification) {
                window.LogViewerNotification.show({
                    message: '复制失败',
                    type: 'error'
                });
            }
    ...
    function performHighlight() {
        ...
        // 在当前行中查找并高亮选中的文字
        const $line = $(`.log-line[data-line="${currentLineNumber}"]`);
        if ($line.length === 0) return;
        
        const $logTxt = $line.find('.log-txt');
        let html = $logTxt.html();
        
        // 移除之前的自定义高亮
        html = html.replace(/<mark class="log-highlight"[^>]*>([^<]*)<\/mark>/g, '$1');
        
        // 添加新的高亮
        const escapedText = currentSelection.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const regex = new RegExp(`(${escapedText})`, 'gi');
        html = html.replace(regex, '<mark class="log-highlight">$1</mark>');
    ...
    function performClearHighlights() {
        // 移除所有自定义高亮
        $('.log-line .log-txt mark.log-highlight').each(function() {
            const $mark = $(this);
    ...
    function updateMarkDisplay() {
        $('.log-line').removeClass('marked');
        markedLines.forEach(lineNumber => {
            $(`.log-line[data-line="${lineNumber}"]`).addClass('marked');
        });
    }
    ...
    function performCopyPath() {
        const filePath = window.LogViewerUIState ? window.LogViewerUIState.getActiveFileName() : null;
        if (!filePath) return;
        ...
        const printStyles = `
            <style id="print-styles">
                @media print {
                    ...
                    /* 语法高亮样式 */
                    .log-keyword { color: #0000ff !important; font-weight: bold; }
                    .log-string { color: #008000 !important; }
                    .log-number { color: #ff6600 !important; }
                    .log-comment { color: #808080 !important; font-style: italic; }
                    .log-error { color: #ff0000 !important; font-weight: bold; }
                    .log-warning { color: #ff8c00 !important; }
                    .log-info { color: #0080ff !important; }
                    .log-debug { color: #808080 !important; }
                    .log-timestamp { color: #666666 !important; }
                    .log-level { font-weight: bold; }
                    .log-level.ERROR { color: #ff0000 !important; }
                    .log-level.WARN { color: #ff8c00 !important; }
                    .log-level.INFO { color: #0080ff !important; }
                    .log-level.DEBUG { color: #808080 !important; }
